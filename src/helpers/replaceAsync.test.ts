...
// @ts-nocheck
import { replaceAsync as replace } from "./replaceAsync"

describe(`Regression tests`, () => {
	test(`Snippet from Readme`, async () => {
		async function getColorByName(name: string) {
			switch (name) {
				case `papayawhip`:
					return `FFEFD5`
				case `rebeccapurple`:
		...
		const spec = `I want background to be #papayawhip and borders #rebeccapurple.`
	...
	test(`Sequence snippet`, async () => {
		...
		expect(log).toEqual(`<p></p><r></r><o></o><t></t><o></o><t></t><y></y><p></p><e></e>`)
	})
	...
	test(`Issue #1 set lastIndex to 0 for regex.`, async () => {
		...
		expect(await replace(x, anchorRegex, () => Promise.resolve(`returned`))).toEqual(
			`Nunquam perdere returned olla returned.`
		)
	})
})
...
describe(`No arguments case`, () => {
	...
	it(`Throws async error`, async () => {
		await expect(replace()).rejects.toThrowError(`Input string is malformed.`)
		// return replace().catch((error) => {
		// 	expect(error.message).toEqual(`Input string is malformed.`)
		// })
	})
})
...
describe(`Single argument case`, () => {
	...
	it(`Converts arbitrary values to strings – int`, async () => {
		expect(await replace(10)).toEqual(`10`)
	})
	it(`Converts arbitrary values to strings – float`, async () => {
		expect(await replace(0.1)).toEqual(`0.1`)
	})
	it(`Converts arbitrary values to strings – object.toString()`, async () => {
		expect(
			await replace({
				toString() {
					return `:)`
...
describe(`No replaceValue`, () => {
	...
	it(`Replaces matches with 'undefined'`, async () => {
		expect(await replace(`prototype`, `p`)).toEqual(`prototype`)
		expect(await replace(`prototype`, /p/)).toEqual(`prototype`)
		expect(await replace(`prototype`, /p/g)).toEqual(`prototype`)
	})
})
...
describe(`Synchronous function replaceValue`, () => {
	...
	it(`Correctily replaces matches`, async () => {
		expect(await replace(`prototype`, `p`, () => `b`)).toEqual(`brototype`)
		expect(await replace(`prototype`, /p/, (match) => `(` + match + `)`)).toEqual(`(p)rototype`)
		expect(await replace(`prototype`, /p/g, (match) => `(` + match + `)`)).toEqual(
			`(p)rototy(p)e`
		)
		expect(
			await replace(`prototype`, /p(.)/g, (match, group) => `p` + group.toUpperCase())
		).toEqual(`pRototypE`)
	})
...
describe(`Asynchronous function replaceValue`, () => {
	...
	it(`Correctily replaces matches`, async () => {
		expect(await replace(`prototype`, `p`, async () => `b`)).toEqual(`brototype`)
		expect(await replace(`prototype`, /p/, async (match) => `(` + match + `)`)).toEqual(
			`(p)rototype`
		)
		expect(await replace(`prototype`, /p/g, async (match) => `(` + match + `)`)).toEqual(
			`(p)rototy(p)e`
		)
		expect(
			await replace(`prototype`, /p(.)/g, async (match, group) => `p` + group.toUpperCase())
		).toEqual(`pRototypE`)
	})
